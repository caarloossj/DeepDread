...
namespace XNodeEditor {
    ...
    public static class NodeEditorReflection {
        [NonSerialized] private static Dictionary<Node, Color> nodeTint;
        ...
        public static bool TryGetAttributeTint(this Node node, out Color tint) {
            if(nodeTint == null)
            {
                tint = default(Color);
                return false;
            }
            if(nodeTint.ContainsKey(node)){
            }
            return nodeTint.TryGetValue(node, out tint);
        }

        public static void TrySetAttributeTint(this Node node, Color newColor) {
            if(nodeTint == null)
            {
                nodeTint = new Dictionary<Node, Color>();
            }

            if(!nodeTint.ContainsKey(node))
            {
                nodeTint.Add(node, newColor);
            } else {
                nodeTint[node] = newColor;
            }
        }

        public static void ClearAttributeTint(this Node node) {
            if(nodeTint != null)
            {
                if(nodeTint.ContainsKey(node))
                {
                    nodeTint.Remove(node);
                }
            }
        }
